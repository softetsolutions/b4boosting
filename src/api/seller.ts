...
export const approveSellerRequest = async (requestId: string): Promise<void> => {
  const { token } = getAuthInfo();

  const response = await fetch(`${API_BASE_URL}/seller/${requestId}/approve`, {
    method: "PATCH",
    headers: {
      Accept: "application/json",
      Authorization: `Bearer ${token}`,
...
export const rejectSellerRequest = async (requestId: string): Promise<void> => {
  ...
  const response = await fetch(`${API_BASE_URL}/seller/${requestId}/reject`, {
    method: "PATCH",
    headers: {
      Accept: "application/json",
      Authorization: `Bearer ${token}`,
